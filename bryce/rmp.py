...
def vectorizeWords(xtrain, xvalid, xtest):    
    ...
    xtrDFWords = pd.DataFrame(xtrWords.A, columns=countVect.get_feature_names())
    xvaDFWords = pd.DataFrame(xvaWords.A, columns=countVect.get_feature_names())
    xteDFWords = pd.DataFrame(xteWords.A, columns=countVect.get_feature_names())
